...
# Carried-over context (verbatim from 2026-06-25 Evening2 — preserved so we don't lose the FOB research thread)
...
## Artifacts / provenance (Evening2)
- EA: [fob_trader.mq5](mt5/Experts/fob_system/fob_trader.mq5) (was v1.8.0 sha 314914c at Evening2; NOW v1.9.0 sha 2592f1c). Emitter [fob_baysix.mq5](mt5/Experts/fob_system/fob_baysix.mq5) pristine. Verified: trader's 2-TF ingest = byte-identical PBO/VR/CF events vs 9-TF emitter (`FobClassifyBreak` only touches `st[etf]` + `st[etf+1]`).
- Results: **result_id 11** (CF_ZONE, WR 29.97%), **result_id 12** (ATR, WR 42.88%). Lineage: **log 72** (PROPOSED exit CF_ZONE→ATR), **log 73** (FALSIFIED entry continuation → PROPOSED fade CF1).
- Ledgers in `Common\Files\FOB\`: `…v170_H1_cf0…` (id 11), `…v180_H1_cfz…` (id 11 re-run), `…v180_H1_atr_cf0…` (id 12).

## Notes (Evening2)
- Tester chart's red/grey Ask/Bid bands annoy Syafiq — cosmetic; fix = clean `tester.tpl` (deferred).
