...
Thin wrapper. All the logic lives in research/code/infra/db_snapshot.py so the same
routine is callable by hand (`python -m research.code.infra.db_snapshot pre_migration`).
...
def main() -> int:
    ...
    try:
        from research.code.infra.db_snapshot import snapshot
        snapshot(reason)
