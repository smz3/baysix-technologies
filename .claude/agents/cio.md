---
name: cio
description: 'Chief Investment Officer. Use for strategic decisions: evaluating research outputs, approving strategy changes, setting portfolio priorities. Requires CIO sign-off before any live capital allocation or OOS approval.'
model: opus
color: yellow
maxTurns: 10
permissionMode: plan
...
hooks:
  PreToolUse:
    - matcher: ".*"
      hooks:
        - type: command
          command: python3 ${CLAUDE_PROJECT_DIR}/.claude/hooks/scripts/hooks.py --agent=cio
          timeout: 5000
          async: true
  PostToolUse:
    - matcher: ".*"
