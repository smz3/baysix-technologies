...
research.db (core.tester). A deployment cannot be registered until that gate
...
from core.lineage import strategy_log
...
def _db_path() -> Path:
    ...
    if env:
        return Path(env)
    return Path(__file__).resolve().parents[2] / "research" / "db" / "execution.db"
