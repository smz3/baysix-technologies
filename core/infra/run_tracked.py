...
USAGE (agent):
    python core/infra/run_tracked.py <name> -- <command...>
e.g.
    python core/infra/run_tracked.py brc_lifecycle -- python -X utf8 research/models/brc/brc001/lifecycle.py

Then wait on the sentinel (existence only):
    until [ -f outputs/_runs/<name>.done ]; do sleep 10; done
...
def main() -> None:
    ...
    print(f"[run_tracked] agent:    until [ -f outputs/_runs/{name}.done ]; do sleep 10; done")
