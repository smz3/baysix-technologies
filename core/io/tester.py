...
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))  # repo root for run-as-script
from core.gates import pipeline
from core.infra.schema_ledger import SCHEMA_MT5
...
from core.infra.db_path import DB_PATH  # noqa: F401  (task 357: one canonical path)
...
def clear_fob_payload_run(run_id: int) -> dict:
    ...
    (core.io.fob_payload.export_run). Keeps the run header (tester_runs) and
