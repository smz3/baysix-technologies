# core/ — module index (Protocol 4.0 lean package)
...
[`__init__.py`](__init__.py) re-exports every public module, so `from core
...
(`from core.gates import pipeline`) to stay immune to `__init__` ordering;
...
| [gates/idea_cli.py](gates/idea_cli.py) | (read-only) | CLI over the getters: `status`, `next <idea>`, `gatecheck`, `prebrief`. Run: `python core/gates/idea_cli.py …`. |
