...
- **Key deltas:**
  ...
  2. **Cost — CORRECTED 2026-06-16 (earlier "3–4×" claim was WRONG):** JM Pro gold spread = ~2 pip = $0.20/oz RT = **0.77 bps at $2.6k gold (~1.1 bps at $1.8k)** vs paper k=0.7 bps RT → only **~1.1–1.6×**, not 3–4×. AND cost is applied to **turnover** (mean |Δw|≈0.066/day [§10]), so effective drag ≈ 0.7bps×0.066 ≈ 0.05 bps/day ≈ **0.12%/yr** — negligible vs 2.62%/yr. **Cost does NOT kill this strategy.** The earlier death-boundary claim leaned on Table 3, which is internally inconsistent (its 1.0× Sharpe 0.94 contradicts headline 2.88) and should not be trusted for absolute figures.
...
#### Verdict
Needs full replication before trusting, and is mis-matched to BRK-001 as briefed. Headline numbers (Sharpe 2.88, MaxDD 0.52%, α 2.25%/yr, SPA p=0.000, reversal Sharpe −2.95) are methodologically clean — strict walk-forward, frozen params, ablation + reversal + SPA falsification, HAC SEs — a textbook validation template worth copying. But the real gates (cost is NOT one — see corrected Context Fit): (1) it is a **trend/momentum regime follower, not a prior-session range breakout** — does not address BRK-001's hypothesis; (2) it trades **daily GC futures**, while we hold **XAUUSD spot tick** — signal resamples to daily and loses roll-P&L; (3) **all profit sits in the long-only bull regime** [Table 2] over a 2015–25 secular gold bull, and the **2.6%/yr real return** needs ~10–16× leverage to matter on a $50 account (collides with our min-lot DD floor). Realized vol 0.91% vs the "implied 43%" headline means the eye-catching CAGR is a leverage-scaling artifact, not realized performance. Use: borrow the engineering scaffold (walk-forward freeze, friction-adjusted Kelly, ATR exits, ablation/SPA suite); do NOT treat its 43%/billion-$ claims as transferable to XAUUSD spot.
