...
- **Wired for BRC-trader + FOB-trader** — both `#include <…_version.mqh>` and print `[… TRADER] v<ver> | git <sha>-DIRTY(exploratory) | built …` on init. (Emitters keep the manual `<SYS>_VERSION` stamp only — they're read-only oracles, re-emittable.)
...
**Tester model (HARD, set 2026-06-29 by Syafiq):** BOTH the **emitter AND the trader** run and are tested on **REAL TICKS only** (Model=4, dukascopy from 2016-06-01). **Open-prices is BANNED for FOB** — FOB is a tick-resolution model (intrabar touch/retest path matters), so an open-prices emitter is invalid, not merely slower. ⚠️ The current emitter code gates its intrabar/forming-bar touch path to LIVE-only and SKIPS it in the tester (`bool live = !MQLInfoInteger(MQL_TESTER)` in [fob_baysix.mq5](mt5/Experts/fob_system/fob_baysix.mq5)) — so running on ticks today still emits bar-resolution output; that gate MUST be removed so the tester resolves true intrabar detection (open backlog task). Trader fills are level-based (limit at zone level) to stay deterministic ([[brc_trader_realtick_model]]). Tester needs UTC offset.
